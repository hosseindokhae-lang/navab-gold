...
// Keep the existing application server untouched. This wrapper only provides
// a more tolerant live-market relay for the public /api/market endpoint.
...
const aliases = {
  gram18: ['geram18','gram18','geram_18','gram_18','18k','gold18'],
  gram740: ['geram740','gram740','geram_740','gram_740','740'],
  gram21: ['geram21','gram21','geram_21','gram_21'],
  gram22: ['geram22','gram22','geram_22','gram_22'],
  usd: ['usd','dollar','dollar_irt','usd_irt'],
  aed: ['aed','dirham','emarat','aed_irt'],
  cad: ['cad','canada','canadian_dollar','cad_irt'],
  eur: ['eur','euro','eur_irt'],
  gbp: ['gbp','pound','pound_irt'],
  chf: ['chf','franc','swiss_franc'],
  cny: ['cny','yuan','yuan_irt'],
  ounceUsd: ['ounce','ounceusd','ounce_usd','ons','ons_usd'],
  mazanehJahani: ['mazaneh-jahani','mazanehJahani','mazaneh_jahani'],
  emami: ['sekkejad','emami','sekke_emami'],
  half: ['sekkenim','half','nim'],
  quarter: ['sekkerob','quarter','rob'],
  gerami: ['sekkegrm','gerami','gerami_coin']
};
...
const proxy = http.createServer((req, res) => {
  ...
  upstream.on('error', () => { if (!res.headersSent) { res.writeHead(502, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('Bad gateway'); } });
  req.pipe(upstream);
});

proxy.listen(PUBLIC_PORT, '0.0.0.0', () => console.log('[NAVAB] proxy listening on', PUBLIC_PORT, '-> app', APP_PORT));
function shutdown() { try { child.kill('SIGTERM'); } catch {} try { talaSocket?.close(); } catch {} process.exit(0); }
